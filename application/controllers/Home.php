...
class Home extends CI_Controller
{
	...
	public function form($id_kelas = null)
{
    // Redirect jika admin
    if ($this->session->userdata('is_admin_login') == TRUE) {
        redirect('admin/dashboard', 'refresh');
    ...
    // Simpan ID kelas ke dalam sesi jika tersedia
    if ($id_kelas) {
        $this->session->set_userdata('selected_kelas', $id_kelas);
    }
    ...
    if ($this->input->post('submit')) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
                ...
                'asal_sekolah' => $this->security->xss_clean($this->input->post('asal_sekolah')),
        ...
        $data['selected_kelas'] = $this->session->userdata('selected_kelas'); // Ambil kelas yang dipilih dari sesi
